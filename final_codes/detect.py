...
from maniquin_drop import maniquin

# ----------------- CONFIG -----------------
MAVLINK_CONN = "udp:127.0.0.1:14552"  #Check out
YOLO_MODEL = "best(S6).pt"
LAST_WAYPOINT = (12.972345, 77.594876)  # need to place the drop point
TARGET_ALT=8.0
SENSOR_WIDTH_MM = 5.6/1000.0
FOCAL_LENGTH_MM = 4.7/1000.0


# ----------------- MAVLINK CONNECTION (vehicle) -----------------
...
flag = False  # payload dropped check
DROP_COOLDOWN = 20   
last_drop_time = 0

# ----------------- SUPPORT FUNCTIONS (kept names similar) -----------------
...
# ----------------- PAYLOAD FUNCTION (same signature) -----------------
    ...
    while time.time() - verify_start < 5:
        ...
        if results and results[0].boxes:
            for box2 in results[0].boxes:
                ...
                cv2.putText(annotated, label, f"{label} {conf:.2f}",(bx1, by1 - 10),
        ...
        if found_pool:
            ...
            pool_box = (x1, y1, x2, y2)   

   
            maniquin(vehicle,cap,model1,pool_box)
            verified = True
            break
        else:
...
while True:
    ...
    if results and results[0].boxes:
        ...
        if pool_detected and not flag:
            ...
            drop_success = payload_drop(vehicle, cap, model, box, frame)
